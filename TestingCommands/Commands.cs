...
namespace TestingCommands
{
    public class Commands : BaseScript
    {
        ...
        private CallbackDelegate NUI_OnChatResult(IDictionary<string, object> data, CallbackDelegate result)
        {
            ...
            if(!string.IsNullOrWhiteSpace(message) && !cancel)
            {
                if (message.StartsWith("/"))
                    ...
                    TriggerEvent("_chat:messageEntered", Game.Player.Name, new byte[] { 0, 0x99, 255 }, message);
            }
            result("ok");
            return result;
        }
        ...
        {
            SendNUIMessage("{\"type\":\"ON_MESSAGE\", \"message\": { \"color\": [" + 
                color[0] + "," + color[1] + "," + color[2] + "], \"multiline\":true, \"args\":[" + 
                (string.IsNullOrWhiteSpace(text) ? "\"" + author + "\"" : "\"" + author + "\"" + "," + "\"" + text + "\"") + "]}}");
        }
